...
class UserTest(APITestCase):
    ...
    def setUp(self):
        self.user = DataSeeder().get_users('user')
    ...
    #     self.client.force_authenticate(user=self.normal_user)
    #     response = self.client.get(f"/tag/{self.normal_user.id}/")
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)

    #     self.assertEqual(response.json()['name'], 'black')
    ...
    #     self.client.force_authenticate(user=self.normal_user)
    #     response = self.client.put(f"/tag/{self.normal_user.id}/", self.updated_tag)
    ...
    #     self.client.force_authenticate(user=self.normal_user)
    #     response = self.client.delete(f"/tag/{self.normal_user.id}/")
